...
Usr ClangType::get_usr_hash() const {
    if (is_builtin()) return static_cast<Usr>(cx_type.kind);
...
std::vector<ClangType> ClangType::get_template_arguments() const {
    ...
    assert(size >= 0);
    if (size < 0) return std::vector<ClangType>();
...
ClangCursor::operator bool() const { return !clang_Cursor_isNull(cx_cursor); }
...
CXCursorKind ClangCursor::get_kind() const { return cx_cursor.kind; }
...
Range ClangCursor::get_spell(CXFile* cx_file) const {
    // TODO for Objective-C methods and Objective-C message expressions, there
    // are multiple pieces for each selector identifier.
...
ClangCursor ClangCursor::template_specialization_to_template_definition()
    ...
    if (definition.kind == CXCursor_FirstInvalid) return cx_cursor;
    return definition;
}
...
std::vector<ClangCursor> ClangCursor::get_arguments() const {
    ...
    if (size < 0) return std::vector<ClangCursor>();
...
bool ClangCursor::is_valid_kind() const {
    ...
    if (clang_Cursor_isNull(referenced)) return false;
...
std::string ClangCursor::get_comments() const {
    ...
    if (clang_Range_isNull(range)) return {};
