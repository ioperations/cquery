...
optional<LsSymbolInformation> GetSymbolInfo(QueryDatabase* db,
                                            WorkingFiles* working_files,
                                            SymbolIdx sym, bool use_short_name);
...
template <typename Fn>
void EachOccurrence(QueryDatabase* db, SymbolIdx sym, bool include_decl,
                    Fn&& fn) {
    WithEntity(db, sym, [&](const auto& entity) {
        for (QueryId::LexicalRef ref : entity.uses) fn(ref);
        if (include_decl) {
            for (auto& def : entity.def)
                if (def.spell) fn(*def.spell);
            for (QueryId::LexicalRef ref : entity.declarations) fn(ref);
        }
    });
}
...
template <typename Fn>
void EachOccurrenceWithParent(QueryDatabase* db, SymbolIdx sym,
                              bool include_decl, Fn&& fn) {
    WithEntity(db, sym, [&](const auto& entity) {
        ...
        for (QueryId::LexicalRef ref : entity.uses) fn(ref, parent_kind);
        if (include_decl) {
            for (auto& def : entity.def)
...
template <typename Fn>
void EachDefinedType(QueryDatabase* db, const std::vector<QueryId::Type>& ids,
                     Fn&& fn) {
...
template <typename Fn>
void EachDefinedFunc(QueryDatabase* db, const std::vector<QueryId::Func>& ids,
                     Fn&& fn) {
...
template <typename Fn>
void EachDefinedVar(QueryDatabase* db, const std::vector<QueryId::Var>& ids,
                    Fn&& fn) {
