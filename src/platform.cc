...
void MakeDirectoryRecursive(const AbsolutePath& path) {
    if (TryMakeDirectory(path)) return;

    std::string prefix = "";
    if (path.path[0] == '/') prefix = "/";
...
optional<std::string> RunExecutable(const std::vector<std::string>& command,
    ...
    // Read stdout and stderr output simultaneously to avoid GCC hang on
    // Windows. See https://github.com/cquery-project/cquery/issues/781 for more
    // details.
    ...
    ec = read_stdout.get();
    if (ec) {
        LOG_S(ERROR) << "Error reading stdout output of "
                     << command_with_error(ec);
        return nullopt;
    }
