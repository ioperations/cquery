...
void PlatformInit() {
    ...
    // `clang++ -E -x c++ - -v < nul` will not find system include directories
    // if ProgramData is not set.
    if (!getenv("ProgramData")) {
        LOG_S(WARNING)
            << "The \"ProgramData\" environment variable is not set. "
...
optional<AbsolutePath> NormalizePath(const std::string& path0,
    ...
    */
    ...
    if (!len) return nullopt;
    path = std::string(buffer, len);

    // Get the actual casing of the path, ie, if the file on disk is `C:\FooBar`
    // and this function is called with `c:\fooBar` this will return
    // `c:\FooBar`. (drive casing is lowercase).
    if (ensure_exists) {
...
bool IsSymLink(const AbsolutePath& path) { return false; }
...
bool RunObjectiveCIndexTests() { return false; }
