...
std::vector<std::string> ExtractSystemIncludePaths(
    ...
    //
    // Parse the output of ie, `clang++ -E -xc++ - -v < nul`. We are looking for
    // a section like this:
    //
    // ...
    ...
    constexpr const char k_include_postfix_to_remove[] =
        " (framework directory)";
    ...
    for (auto& line : lines) {
        ...
        if (EndsWith(line, k_include_postfix_to_remove)) {
            line = line.substr(
                0, line.size() - sizeof(k_include_postfix_to_remove) + 1);
...
std::vector<std::string> ExtractSystemDefines(
    ...
    // https://github.com/cquery-project/cquery/issues/824. This can be
    // re-enabled on master after the reported issues are investigated.
    return {};
    ...
    for (auto& line : lines) {
      ...
      if (value.find(" ") == std::string::npos) {
        output.emplace_back("-D" + line.substr(8, space_index - 8) + "=" +
    value); } else { output.emplace_back("-D'" + line.substr(8, space_index - 8)
    + "=" + value + "'");
      }
    }
...
std::vector<std::string> FindSystemIncludeDirectories(
    ...
    const std::string& language, const std::string& working_directory,
    ...
    for (const std::string& compiler_driver : compiler_drivers) {
        ...
        compiler_type compiler_type = FindCompilerType(compiler_driver);
        ...
        if (!clang_output || clang_output->empty()) continue;

        LOG_S(INFO) << "Output:\n" << Trim(*clang_output);
        std::vector<std::string> paths =
            ExtractSystemIncludePaths(*clang_output);
...
std::vector<std::string> FindSystemDefines(
    ...
    for (const std::string& compiler_driver : compiler_drivers) {
        ...
        if (!preprocessor_output || preprocessor_output->empty()) continue;

        LOG_S(INFO) << "Output:\n" << Trim(*preprocessor_output);
        std::vector<std::string> defines =
            ExtractSystemDefines(*preprocessor_output);
        if (!defines.empty()) return defines;
    }
    return {};
}
