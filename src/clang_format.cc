...
std::vector<LsTextEdit> ConvertReplacementsToTextEdits(
    const std::string& document, const std::vector<Replacement>& replacements) {
    std::vector<LsTextEdit> edits;
    for (const Replacement& replacement : replacements) {
        LsTextEdit edit;
        edit.range.start = GetPositionForOffset(replacement.offset, document);
        edit.range.end = GetPositionForOffset(
            replacement.offset + replacement.length, document);
        edit.new_text = replacement.text;
        LOG_S(INFO) << "Text edit from " << edit.range.start.ToString()
                    << " to " << edit.range.end.ToString() << " with text |"
                    << edit.new_text << "|";
    ...
    for (const std::string& clang_format_driver : clang_format_drivers) {
        ...
        if (start_offset.has_value()) {
            ...
            args.push_back("-length=" +
                           std::to_string(*end_offset - *start_offset));
        }

        optional<std::string> output = RunExecutable(args, file_contents);
        if (!output || output->empty()) continue;
        // Do not check if replacements is empty, since that may happen if there
