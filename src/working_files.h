...
struct WorkingFile {
    ...
    // Empty indicates either buffer or index has been changed and
    // re-computation is required. For index_to_buffer[i] == j, if j >= 0, we
    // are confident that index line i maps to buffer line j; if j == -1,
    // FindMatchingLine will use the nearest confident lines to resolve its line
    // number.
    ...
    // lock!
    std::vector<lsDiagnostic> m_diagnostics;

    WorkingFile(const AbsolutePath& filename,
    ...
    // TODO: Move FindClosestCallNameInBuffer and FindStableCompletionSource
    // into lex_utils.h/cc
    ...
    // Returns a relatively stable completion position (it jumps back until
    ...
    LsPosition FindStableCompletionSource(
...
struct WorkingFiles {
    ...
    // Run |action| on the file identified by |filename|. This executes under
    // the lock.
    ...
    // strings. For example, {"foo", "bar"} means that every result has either
    // the string "foo" or "bar" contained within it.
