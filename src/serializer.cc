...
void Reflect(Writer& visitor, uint8_t& value) { visitor.Int(value); }

void Reflect(Reader& visitor, short& value) {
    if (!visitor.IsInt()) throw std::invalid_argument("short");
...
void Reflect(Reader& visitor, unsigned short& value) {
    if (!visitor.IsInt()) throw std::invalid_argument("unsigned short");
...
void Reflect(Reader& visitor, unsigned& value) {
    if (!visitor.IsUint64()) throw std::invalid_argument("unsigned");
...
void Reflect(Reader& visitor, JsonNull& value) { visitor.GetNull(); }
...
void Reflect(Writer& visitor, IndexInclude& value) {
    ...
    if (g_test_output_mode) {
        ...
        if (!StartsWith(value.resolved_path, "&")) basename = "&" + basename;
...
void ReflectHoverAndComments(Writer& visitor, Def& def) {
    ...
    if (!g_test_output_mode || !def.hover.empty())
        ReflectMember(visitor, "hover", def.hover);
    if (!g_test_output_mode || !def.comments.empty())
        ReflectMember(visitor, "comments", def.comments);
}
...
void ReflectShortName(Writer& visitor, Def& def) {
    if (g_test_output_mode) {
        std::string short_name(def.detailed_name.substr(def.short_name_offset,
                                                        def.short_name_size));
        ReflectMember(visitor, "short_name", short_name);
    } else {
        ReflectMember(visitor, "short_name_offset", def.short_name_offset);
...
void Reflect(TVisitor& visitor, IndexFile& value) {
    ...
    if (!g_test_output_mode) REFLECT_MEMBER(dependencies);
...
void Reflect(Reader& visitor, serialize_format& value) {
    std::string fmt = visitor.GetString();
    value =
        fmt[0] == 'm' ? serialize_format::MessagePack : serialize_format::Json;
}

void Reflect(Writer& visitor, serialize_format& value) {
    switch (value) {
        case serialize_format::Json:
            visitor.String("json");
            break;
        case serialize_format::MessagePack:
            visitor.String("msgpack");
            break;
    }
}

std::string Serialize(serialize_format format, IndexFile& file) {
    switch (format) {
        case serialize_format::Json: {
...
std::unique_ptr<IndexFile> Deserialize(
    serialize_format format, const AbsolutePath& path,
    const std::string& serialized_index_content,
    const std::string& file_content, optional<int> expected_version) {
    ...
    switch (format) {
        case serialize_format::Json: {
            ...
            if (g_test_output_mode || !expected_version) {
...
void SetTestOutputMode() { g_test_output_mode = true; }
