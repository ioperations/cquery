...
struct Config {
    ...
    // takes only 60% of the corresponding JSON size, but is difficult to
    // inspect. msgpack does not store map keys and you need to re-index
    // whenever a struct member has changed.
    serialize_format cacheFormat = serialize_format::Json;

    // Value to use for clang -resource-dir if not present in
    // compile_commands.json.
    //
    // cquery includes a resource directory, this should not need to be
    // configured unless you're using an esoteric configuration. Consider
    // reporting a bug and fixing upstream instead of configuring this.
    //
    ...
    // |clang++ -E -xc++ - -v|?
    //
    // System includes are necessary to resolve system/stl headers like
    // <vector>.
    //
    // cquery will try a number of different compilers for system include
    // extraction. If there is a compiler mentioned in your project, that will
    // be used. Otherwise, cquery will use cquery-clang, and then the
    // system-wide clang++ or g++ will be used.
    ...
    // interval; it could take significantly longer if cquery is completely
    // idle.
    int progressReportFrequencyMs = 500;

    // If true, cquery will emit $cquery/queryDbStatus notifications whenever
    // the querydb thread is busy or idle.
    ...
    struct Completion {
        ...
        // just contains the completion item parent context. Also, in this mode,
        // functions with default arguments, generates one more item per default
        // argument so that the right function call can be selected. That is,
        ...
        // cquery receives multiple completion requests while completion is
        // still running it will only service the newest request. If this is set
        // to false then all completion requests will be serviced.
        bool dropOldRequests = true;

        // If true, filter and sort completion response. cquery filters and
        // sorts completions to try to be nicer to clients that can't handle big
        ...
        // this will be elided with ".." put at the front. Set to 0 or a
        // negative number to disable eliding.
    ...
    struct Index {
        ...
        // If true, project paths that were skipped by the whitelist/blacklist
        // will be logged.
...
MAKE_REFLECT_STRUCT(Config::Completion, enableSnippets, detailedLabel,
                    dropOldRequests, filterAndSort, includeMaxPathSize,
                    includeSuffixWhitelist, includeBlacklist, includeWhitelist);
MAKE_REFLECT_STRUCT(Config::Formatting, enabled)
MAKE_REFLECT_STRUCT(Config::Diagnostics, blacklist, whitelist, frequencyMs,
                    onParse, onType)
...
MAKE_REFLECT_STRUCT(Config::Index, attributeMakeCallsToCtor, blacklist,
                    whitelist, comments, enabled, logSkippedPaths, threads);
...
MAKE_REFLECT_STRUCT(Config, compilationDatabaseCommand,
                    ...
                    discoverSystemIncludes, extraClangArguments,

                    progressReportFrequencyMs, emitQueryDbBlocked,
                    emitInactiveRegions,

                    showDocumentLinksOnIncludes,

                    codeLens, completion, formatting, diagnostics, highlight,
