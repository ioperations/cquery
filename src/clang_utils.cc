...
optional<lsDiagnostic> BuildAndDisposeDiagnostic(CXDiagnostic diagnostic,
    ...
    for (unsigned i = 0; i < num_ranges; i++) {
        ...
        clang_getSpellingLocation(clang_getRangeEnd(range), &file1, &line1,
                                  &column1, nullptr);
        if (file0 != file1 || file0 != file) continue;
        if (line0 < start_line ||
            (line0 == start_line && column0 < start_column)) {
            start_line = line0;
    ...
    for (unsigned i = 0; i < num_fixits; ++i) {
        ...
        CXString text =
            clang_getDiagnosticFixIt(diagnostic, i, &replacement_range);

        LsTextEdit edit;
        edit.new_text = ToString(text);
