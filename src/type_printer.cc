...
int GetNameInsertingPosition(const std::string& type_desc,
    ...
    // should be inserted.
    int ret = 0;
    while (return_type[ret] == type_desc[ret]) ret++;
...
std::string GetFunctionSignature(IndexFile* db, NamespaceHelper* ns,
    ...
    if (type_desc[function_name_offset] == '(') {
        ...
        // Second pass: insert argument names before each comma and closing
        // paren.
        ...
        for (auto& arg : args) {
            ...
            // TODO Use inside-out syntax. Note, clang/lib/AST/TypePrinter.cpp
            // does not print arg names.
            type_desc_with_names.insert(type_desc_with_names.end(),
                                        &type_desc[i], &type_desc[arg.first]);
            i = arg.first;
        ...
        // type_desc is either a typedef, or some complicated type we cannot
        // handle. Append the function_name in this case.
