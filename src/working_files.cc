...
int MyersDiff(const char* a, int la, const char* b, int lb, int threshold) {
    ...
    static int v_static[2 * k_max_column_align_size + 2];
    ...
    if (la + lb > 2 * k_max_column_align_size)
...
int AlignColumn(const std::string& a, int column, std::string b, bool is_end) {
    ...
    if ((int)a.size() - tail < column) return column + b.size() - a.size();
    if (std::max(a.size(), b.size()) - head - tail >= k_max_column_align_size)
...
optional<int> WorkingFile::GetBufferPosFromIndexPos(int line, int* column,
    ...
    // lookup the line contents in the indexed file contents, and try to find
    // the most similar line in the current buffer file.
    //
...
optional<int> WorkingFile::GetIndexPosFromBufferPos(int line, int* column,
    ...
    if (line < 0 || line >= (int)buffer_lines.size()) return nullopt;
...
std::string WorkingFile::FindClosestCallNameInBuffer(
    ...
    *active_parameter = 0;
    ...
    if (offset > 0 && buffer_content[offset] == ')') --offset;
    ...
    while (offset > 0) {
        ...
        if (balance == 0 && c == ',') *active_parameter += 1;
...
LsPosition WorkingFile::FindStableCompletionSource(
    ...
    std::string* existing_completion, LsPosition* replace_end_position) const {
    *is_global_completion = true;
    ...
    while (offset > 0) {
        char c = buffer_content[offset - 1];
        if (!isalnum(c) && c != '_') {
            // Global completion is everything except for dot (.), arrow (->),
            // and double colon (::)
            if (c == '.') *is_global_completion = false;
            if (offset > 2) {
...
WorkingFile* WorkingFiles::GetFileByFilenameNoLock(
    ...
    for (auto& file : files) {
        if (file->filename == filename) return file.get();
    }
    return nullptr;
}
...
void WorkingFiles::OnChange(const LsTextDocumentDidChangeParams& change) {
    ...
    AbsolutePath filename = change.text_document.uri.GetAbsolutePath();
    WorkingFile* file = GetFileByFilenameNoLock(filename);
    if (!file) {
    ...
    if (change.text_document.version)
        file->version = *change.text_document.version;

    for (const LsTextDocumentContentChangeEvent& diff :
...
LsPosition CharPos(const WorkingFile& file, char character,
...
TEST_SUITE("WorkingFile") {
    ...
    TEST_CASE("nested call") {
        ...
        REQUIRE(f.FindClosestCallNameInBuffer(CharPos(f, 'g'), &active_param) ==
                "abcd");
        REQUIRE(active_param == 0);
        REQUIRE(f.FindClosestCallNameInBuffer(CharPos(f, 'g', 1),
                                              &active_param) == "efg");
        REQUIRE(active_param == 0);
        REQUIRE(f.FindClosestCallNameInBuffer(CharPos(f, 'g', 2),
                                              &active_param) == "efg");
        REQUIRE(active_param == 0);
        REQUIRE(f.FindClosestCallNameInBuffer(CharPos(f, ','), &active_param) ==
                "abcd");
