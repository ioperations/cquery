...
optional<LsRange> ExtractQuotedRange(int line_number, const std::string& line) {
    ...
    if (start == (int)line.size()) return nullopt;
    ...
    return LsRange(LsPosition(line_number, start),
                   LsPosition(line_number, end));
}
...
void LexFunctionDeclaration(const std::string& buffer_content,
    ...
    if (parse_return_type) {
        ...
        while (return_start > 0) {
            ...
            if (paren_balance == 0 && angle_balance == 0) {
                ...
                if (!isspace(c)) expect_token = false;
            }
    ...
    *newlines_after_name = 0;
    ...
    if (type_name && !type_name->empty()) result += *type_name + "::";
...
std::string_view LexIdentifierAroundPos(LsPosition position,
    ...
    for (; end < (int)content.size(); end++)
        if (c = content[end], !(isalnum(c) || c == '_')) break;
...
std::pair<bool, int> CaseFoldingSubsequenceMatch(std::string_view search,
    ...
    bool hasUppercaseLetter =
        std::any_of(search.begin(), search.end(), isupper);
    int skip = 0;
    size_t j = 0;
    for (char c : search) {
        while (j < content.size() &&
            ...
            ++j, ++skip;
        if (j == content.size()) return {false, skip};
        ++j;
    }
    return {true, skip};
}
...
TEST_SUITE("Substring") {
    TEST_CASE("skip") {
        REQUIRE(CaseFoldingSubsequenceMatch("a", "a") ==
                std::make_pair(true, 0));
        REQUIRE(CaseFoldingSubsequenceMatch("b", "a") ==
                std::make_pair(false, 1));
        REQUIRE(CaseFoldingSubsequenceMatch("", "") == std::make_pair(true, 0));
        REQUIRE(CaseFoldingSubsequenceMatch("a", "ba") ==
...
TEST_SUITE("LexFunctionDeclaration") {
    TEST_CASE("simple") {
        ...
        LexFunctionDeclaration(buffer_content, declaration, nullopt,
...
TEST_SUITE("LexWordAroundPos") {
    TEST_CASE("edges") {
        ...
        REQUIRE(LexIdentifierAroundPos(CharPos(content, 'F'), content) ==
                "Foobar");
        REQUIRE(LexIdentifierAroundPos(CharPos(content, 'o'), content) ==
                "Foobar");
        REQUIRE(LexIdentifierAroundPos(CharPos(content, 'b'), content) ==
                "Foobar");
        REQUIRE(LexIdentifierAroundPos(CharPos(content, 'a'), content) ==
                "Foobar");
        REQUIRE(LexIdentifierAroundPos(CharPos(content, 'r'), content) ==
                "Foobar");
    }
