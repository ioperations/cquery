...
std::vector<QueryId::LexicalRef> GetDeclarations(const std::vector<Id<T>>& ids,
                                                 Fn&& fetch_definition) {
...
optional<QueryId::LexicalRef> GetDefinitionExtent(QueryDatabase* db,
    ...
    if (sym.kind == SymbolKind::File)
        return QueryId::LexicalRef(Range(Position(0, 0), Position(0, 0)),
                                   sym.id, sym.kind, role::None,
                                   QueryId::File(sym.id));
    Maybe<QueryId::LexicalRef> ret;
    EachEntityDef(db, sym,
                  [&](const auto& def) { return !(ret = def.extent); });
    return ret;
}
...
optional<QueryId::File> GetDeclarationFileForSymbol(QueryDatabase* db,
    ...
    switch (sym.kind) {
        ...
        case SymbolKind::Func: {
            ...
            if (!func.declarations.empty()) return func.declarations[0].file;
            if (const auto* def = func.AnyDef()) return def->file;
            break;
        }
        case SymbolKind::Type: {
            if (const auto* def = db->GetType(sym).AnyDef()) return def->file;
            break;
        }
        case SymbolKind::Var: {
            if (const auto* def = db->GetVar(sym).AnyDef()) return def->file;
            break;
        }
...
std::vector<QueryId::LexicalRef> GetDeclarations(
    QueryDatabase* db, const std::vector<QueryId::Func>& ids) {
    return GetDeclarations(
        ids, [&](QueryId::Func id) -> QueryFunc& { return db->GetFunc(id); });
}

std::vector<QueryId::LexicalRef> GetDeclarations(
    QueryDatabase* db, const std::vector<QueryId::Type>& ids) {
    return GetDeclarations(
        ids, [&](QueryId::Type id) -> QueryType& { return db->GetType(id); });
}

std::vector<QueryId::LexicalRef> GetDeclarations(
    QueryDatabase* db, const std::vector<QueryId::Var>& ids) {
    return GetDeclarations(
        ids, [&](QueryId::Var id) -> QueryVar& { return db->GetVar(id); });
}
...
optional<LsPosition> GetLsPosition(WorkingFile* working_file,
    ...
    if (!working_file) return LsPosition(position.line, position.column);

    int column = position.column;
    if (optional<int> start = working_file->GetBufferPosFromIndexPos(
            position.line, &column, false))
...
optional<LsRange> GetLsRange(WorkingFile* working_file, const Range& location) {
    ...
    optional<int> end = working_file->GetBufferPosFromIndexPos(
...
ls_symbol_kind GetSymbolKind(QueryDatabase* db, SymbolIdx sym) {
    ls_symbol_kind ret;
    if (sym.kind == SymbolKind::File)
        ret = ls_symbol_kind::File;
    else {
        ret = ls_symbol_kind::Unknown;
    ...
    switch (sym.kind) {
        ...
        default: {
            ...
            EachEntityDef(db, sym, [&](const auto& def) {
                ...
                info.kind = def.kind;
                info.container_name = def.detailed_name;
                return false;
            });
            return info;
        }
...
std::vector<QueryId::SymbolRef> FindSymbolsAtLocation(WorkingFile* working_file,
    ...
    if (working_file) {
        ...
        if (index_line) target_line = *index_line;
    }
    ...
    // Var currently. If a macro expands to tokens led by a SymbolKind::Type,
    // the macro and the Type have the same range. We want to find the macro
