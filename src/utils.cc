...
uint64_t HashUsr(std::string_view s) {
    ...
    (void)siphash(reinterpret_cast<const uint8_t*>(s.data()), s.length(), k,
                  out, 8);
    return ret;
}
...
std::string StripFileType(const std::string& path) {
    ...
    if (last_period != std::string::npos) return path.substr(0, last_period);
    return path;
}
...
static void GetFilesInFolderHelper(
    std::string folder, bool recursive, std::string output_prefix,
    ...
    while (!q.empty()) {
        ...
        while (dir.has_next) {
            ...
            //
            // Note that in the future if we do support dot directories/files,
            // we must always ignore the '.' and '..' directories otherwise this
            // will loop infinitely.
            if (file.name[0] != '.' || strcmp(file.name, ".cquery") == 0) {
                if (file.is_dir) {
                    if (recursive) {
                        std::string child_dir =
                            q.front().second + file.name + "/";
        ...
        bailfile:
            if (tinydir_next(&dir) == -1) {
                LOG_S(WARNING)
                    << "Unable to fetch next file when reading directory "
                    << folder;
                goto bail;
            }
        }
...
std::istream& SafeGetline(std::istream& is, std::string& t) {
    ...
    // that uses streambuf this way must be guarded by a sentry object. The
    // sentry object performs various tasks, such as thread synchronization and
...
std::vector<std::string> ReadLinesWithEnding(const AbsolutePath& filename) {
    ...
    for (std::string line; SafeGetline(input, line);) result.push_back(line);
...
std::vector<std::string> ToLines(const std::string& content,
    ...
    auto remove_last_cr = [](std::string& s) {
        if (!s.empty() && *s.rbegin() == '\r') s.pop_back();
    };
...
std::string FormatMicroseconds(long long microseconds) {
    ...
    while (remaining >= 100) remaining /= 10;

    return std::to_string(milliseconds) + "." + std::to_string(remaining) +
           "ms";
}
...
AbsolutePath GetExecutablePathNextToCqueryBinary(const std::string& name) {
    std::string executable_path = GetExecutablePath().path;
    size_t pos = executable_path.find_last_of('/');
    return AbsolutePath::BuildDoNotUse(executable_path.substr(0, pos + 1) +
                                       name);
}
