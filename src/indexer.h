...
struct Id {
    ...
    template <typename U, typename std::enable_if<std::is_void<T>::value ||
...
struct IndexSymbolRef : Reference {
    ...
    IndexSymbolRef(Range range, AnyId id, SymbolKind kind, enum role role)
...
struct IndexLexicalRef : Reference {
    ...
    IndexLexicalRef(Range range, AnyId id, SymbolKind kind, enum role role)
        : Reference{range, id, kind, role} {}
};
...
struct TypeDefDefinitionData {
    ...
    // While a class/type can technically have a separate
    // declaration/definition, it doesn't really happen in practice. The
    // declaration never contains comments or insightful information. The user
    // always wants to jump from the declaration to the definition - never the
    // other way around like in functions and (less often) variables.
    //
    ...
    // If set, then this is the same underlying type as the given value (ie,
    // this type comes from a using or typedef statement).
...
struct FuncDefDefinitionData {
    ...
    std::string_view DetailedName(bool params) const {
        if (params) return detailed_name;
...
struct VarDefDefinitionData {
    ...
    std::string DetailedName(bool qualified) const {
        if (qualified) return detailed_name;
        ...
        return detailed_name.substr(0, i) +
               detailed_name.substr(short_name_offset);
    }
};
...
struct IndexFile {
    ...
    // JSON has good forward compatibility because field addition/deletion do
    // not harm but currently no efforts have been made to make old MessagePack
    // cache files accepted by newer cquery.
    ...
    // The path to the translation unit cc file which caused the creation of
    // this IndexFile. When parsing a translation unit we generate many
    // IndexFile instances (ie, each header has a separate one). When the user
    // edits a header we need to lookup the original translation unit and
    // reindex that.
...
optional<std::vector<std::unique_ptr<IndexFile>>> Parse(
    FileConsumerSharedState* file_consumer_shared, const std::string& file,
