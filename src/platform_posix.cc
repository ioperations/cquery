...
optional<AbsolutePath> RealPathNotExpandSymlink(std::string path,
    ...
    if (path[0] == '/') {
        ...
        if (!getcwd(tmp, sizeof tmp) && ensure_exists) return nullopt;
        resolved = tmp;
    }
    ...
    while (i < path.size()) {
        ...
        resolved += next_token;
        if (stat(resolved.c_str(), &sb) != 0 && ensure_exists) return nullopt;
    ...
    if (resolved.size() > 1 && resolved.back() == '/') resolved.pop_back();
...
AbsolutePath GetWorkingDirectory() {
    ...
    if (!getcwd(result, sizeof(result))) return AbsolutePath("");
...
bool TryMakeDirectory(const AbsolutePath& absolute_path) {
    const mode_t k_mode = 0777;  // UNIX style permissions
    if (mkdir(absolute_path.path.c_str(), k_mode) == -1) {
    ...
    if (stat(absolute_path.path.c_str(), &buf) != 0) {
        switch (errno) {
            case ENOENT:
                ...
                return nullopt;
            case EINVAL:
                // std::cerr << "GetLastModificationTime: invalid param to _stat
                // for file file " << absolute_path << std::endl;
                return nullopt;
            default:
...
std::vector<const char*> GetPlatformClangArguments() { return {}; }
...
void TraceMe() {
    ...
    // In gdb, you need to invoke `signal SIGCONT` if you want cquery to
    // continue after detaching.
    if (getenv("CQUERY_TRACEME")) raise(SIGTSTP);
}
