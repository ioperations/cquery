...
struct OutProgress : public LsOutMessage<OutProgress> {
    struct Params {
...
MAKE_REFLECT_STRUCT(OutProgress::Params, indexRequestCount, doIdMapCount,
                    onIdMappedCount, onIndexedCount, activeThreads);
MAKE_REFLECT_STRUCT(OutProgress, jsonrpc, method, params);
...
constexpr int k_iteration_size = 200;
struct IterationLoop {
    int remaining = k_iteration_size;
    void IncreaseCount() { remaining *= 50; }

    bool Next() { return remaining-- > 0; }
    void Reset() { remaining = k_iteration_size; }
};
...
struct ActiveThread {
    ActiveThread(ImportPipelineStatus* status) : status_(status) {
        if (g_config->progressReportFrequencyMs < 0) return;
...
ChangeResult ComputeChangeStatus(
    ...
    IndexFile* opt_previous_index, const AbsolutePath& path,
    const std::vector<std::string>& args, const optional<AbsolutePath>& from) {
    auto unwrap_opt = [](const optional<AbsolutePath>& opt) -> std::string {
        if (opt) return " (via " + opt->path + ")";
        return "";
    };
    ...
    // Cannot find file.
    if (!modification_timestamp) return ChangeResult::kDeleted;
    ...
    if (opt_previous_index) {
        if (HashArguments(args) != opt_previous_index->args_hash) {
            LOG_S(INFO) << "Arguments have changed for " << path
                        << unwrap_opt(from);
            return ChangeResult::kYes;
        }
    }
...
CacheLoadResult TryLoadFromCache(
    ...
    const Project::Entry& entry, const AbsolutePath& path_to_index) {
    IndexFile* previous_index = cache_manager->TryLoad(path_to_index);
    if (!previous_index) return CacheLoadResult::kParse;
    ...
    for (const AbsolutePath& dependency : previous_index->dependencies) {
        ...
        if (ComputeChangeStatus(timestamp_manager,
                                modification_timestamp_fetcher, cache_manager,
                                previous_index, dependency, entry.args,
                                previous_index->path) == ChangeResult::kYes) {
            needs_reparse = true;

            // Do not break here, as we need to update |file_consumer_shared|
            // for every dependency that needs to be reparsed.
            file_consumer_shared->Reset(dependency);
        }
    }

    // FIXME: should we still load from cache?
    if (needs_reparse) return CacheLoadResult::kParse;
    ...
    auto try_add_result = [&](Index_DoIdMap request) {
        ...
        if (did_set) result.push_back(std::move(request));
    };
    ...
    for (const AbsolutePath& dependency : previous_index->dependencies) {
        ...
        if (!file_consumer_shared->Mark(dependency)) continue;
...
void ParseFile(DiagnosticsEngine* diag_engine, WorkingFiles* working_files,
               FileConsumerSharedState* file_consumer_shared,
               TimestampManager* timestamp_manager,
               IModificationTimestampFetcher* modification_timestamp_fetcher,
               ImportManager* import_manager, IIndexer* indexer,
               const Index_Request& request, const Project::Entry& entry) {
    // If the file is inferred, we may not actually be able to parse that file
    // directly (ie, a header file, which are not listed in the project). If
    // this file is inferred, then try to use the file which originally imported
    // it.
    ...
    if (entry.is_inferred) {
        ...
        if (entry_cache) path_to_index = entry_cache->import_file;
    }
    ...
    auto indexes = indexer->Index(file_consumer_shared, path_to_index,
                                  entry.args, file_contents);
    ...
    // Add the set of indexes we want to actually import from the index
    // operation.
    for (std::unique_ptr<IndexFile>& new_index : *indexes) {
        ...
        // We did not change the status, which means the import is currently in
        // the pipeline. Drop the request.
        // TODO: add file to a queue which will be processed after import is
        // done?
        if (!did_set) {
        ...
        // Only emit diagnostics for non-interactive sessions, which makes it
        // easier to identify indexing problems. For interactive sessions,
        // diagnostics are handled by code completion.
        if (!request.is_interactive) {
            diag_engine->Publish(working_files, new_index->path,
                                 new_index->diagnostics_);
        }

        // When main thread does IdMap request it will request the previous
        // index if needed.
        LOG_S(INFO) << "Emitting index result for " << new_index->path;
        result.push_back(
            Index_DoIdMap(std::move(new_index), request.cache_manager,
    ...
    for (Index_DoIdMap& request : result) {
        PipelineStatus status =
            import_manager->GetStatus(request.current->path);
...
bool IndexMain_DoParse(
    DiagnosticsEngine* diag_engine, WorkingFiles* working_files,
    FileConsumerSharedState* file_consumer_shared,
    TimestampManager* timestamp_manager,
    IModificationTimestampFetcher* modification_timestamp_fetcher,
    ImportManager* import_manager, IIndexer* indexer) {
...
bool IndexMain_DoCreateIndexUpdate(TimestampManager* timestamp_manager) {
    ...
    while (loop.Next()) {
        ...
        IndexUpdate update = IndexUpdate::CreateDelta(
            previous_id_map, response->current->ids.get(), previous_index,
            response->current->file.get());
        LOG_S(INFO) << "Built index update for "
                    << response->current->file->path
                    << " (is_delta=" << !!response->previous << ")";

        IndexOnIndexed reply(std::move(update));
        const int k_max_size_for_querydb = 1000;
        ThreadedQueue<IndexOnIndexed>& q =
            queue->on_indexed_for_querydb.Size() < k_max_size_for_querydb
...
bool IndexMergeIndexUpdates() {
    ...
    IterationLoop loop;
    while (loop.Next()) {
        optional<IndexOnIndexed> to_join =
            queue->on_indexed_for_merge.TryDequeue(false /*priority*/);
        if (!to_join) break;
        did_merge = true;
    ...
    const int k_max_size_for_querydb = 10;
    ThreadedQueue<IndexOnIndexed>& q =
        queue->on_indexed_for_querydb.Size() < k_max_size_for_querydb
            ? queue->on_indexed_for_querydb
            : queue->on_indexed_for_merge;
...
void Indexer_Main(DiagnosticsEngine* diag_engine,
    ...
    while (true) {
...

        {
            ...
            // IndexMain_DoCreateIndexUpdate for better icache behavior. We need
            // to have some threads spinning on both though otherwise memory
            // usage will get bad.

            // We need to make sure to run both IndexMain_DoParse and
            // IndexMain_DoCreateIndexUpdate so we don't starve querydb from
            // doing any work. Running both also lets the user query the
            // partially constructed index.
            did_work = IndexMain_DoParse(
                           diag_engine, working_files, file_consumer_shared,
                           timestamp_manager, &modification_timestamp_fetcher,
                           import_manager, indexer.get()) ||
                       did_work;

            did_work =
                IndexMain_DoCreateIndexUpdate(timestamp_manager) || did_work;

            // Nothing to index and no index updates to create, so join some
            // already created index updates to reduce work on querydb thread.
            if (!did_work) did_work = IndexMergeIndexUpdates() || did_work;

            // Our completion cache might not be correct once we've indexed a
            // file so it should be cleared.
            if (did_work) {
...
namespace {
void QueryDbDoIdMap(QueueManager* queue, QueryDatabase* db,
                    ImportManager* import_manager, Index_DoIdMap* request) {
...
void QueryDbOnIndexed(QueueManager* queue, QueryDatabase* db,
                      ImportManager* import_manager,
                      ImportPipelineStatus* status,
                      SemanticHighlightSymbolCache* semantic_cache,
                      WorkingFiles* working_files, IndexOnIndexed* response) {
    Timer time;
    db->ApplyIndexUpdate(&response->update);
    time.ResetAndPrint(
        "Applying index update for " +
        std::to_string(response->update.files_def_update.size()) + " files");
    ...
    for (auto& updated_file : response->update.files_def_update) {
        ...
        if (working_file) {
            ...
            // Inactive lines.
            EmitInactiveLines(working_file,
                              updated_file.value.inactive_regions);
...
bool QueryDb_ImportMain(QueryDatabase* db, ImportManager* import_manager,
    ...
    while (loop.Next()) {
        ...
        if (!request) break;
        did_work = true;
        QueryDbDoIdMap(queue, db, import_manager, &*request);
    }

    loop.Reset();
    while (loop.Next()) {
        optional<IndexOnIndexed> response =
            queue->on_indexed_for_querydb.TryDequeue(true /*priority*/);
        if (!response) break;
        did_work = true;
        QueryDbOnIndexed(queue, db, import_manager, status, semantic_cache,
...
TEST_SUITE("ImportPipeline") {
    ...
    TEST_CASE_FIXTURE(Fixture, "FileNeedsParse") {
        auto check = [&](const std::string& file, bool is_dependency = false,
            ...
            return ComputeChangeStatus(
                &timestamp_manager, &modification_timestamp_fetcher,
                cache_manager, opt_previous_index.get(),
                AbsolutePath(file, false /*validate*/), new_args, from);
        };

        // A file with no timestamp is not imported, since this implies the file
        // no longer exists on disk.
        modification_timestamp_fetcher.entries["bar.h"] = nullopt;
        REQUIRE(check("bar.h", false /*is_dependency*/) ==
                ChangeResult::kDeleted);
        ...
        REQUIRE(check("aa.cc", false /*is_dependency*/,
                      false /*is_interactive*/, {"b"} /*old_args*/,
    ...
    TEST_CASE_FIXTURE(Fixture, "one index request") {
        indexer =
            IIndexer::MakeTestIndexer({IIndexer::TestEntry{"foo.cc", 100}});
