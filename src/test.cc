...
void Write(const std::vector<std::string>& strs) {
    for (const std::string& str : strs) std::cout << str << std::endl;
}
  ...
  */
    ...
    // Scan for EXTRA_FLAGS:
    {
        ...
        for (std::string line : lines_with_endings) {
            ...
            if (in_output) flags->push_back(line);
        }
    }
    ...
    // Scan for OUTPUT:
    {
        ...
        for (std::string line_with_ending : lines_with_endings) {
            if (StartsWith(line_with_ending, "*/")) break;
...
void VerifySerializeToFrom(IndexFile* file) {
    ...
    std::string serialized = Serialize(serialize_format::Json, *file);
    std::unique_ptr<IndexFile> result = Deserialize(
        serialize_format::Json, AbsolutePath::BuildDoNotUse("--.cc"),
...
std::string FindExpectedOutputForFilename(
    ...
    for (const auto& entry : expected) {
        if (EndsWith(entry.first, filename)) return entry.second;
    }
...
IndexFile* FindDbForPathEnding(
    ...
    for (auto& db : dbs) {
        if (EndsWith(db->path.path, path)) return db.get();
    }
    return nullptr;
}
...
bool RunIndexTests(const std::string& filter_path, bool enable_update) {
    ...
    if (clang_version != "7.0.0") {
        std::cerr << "Index tests must be run using clang version \""
                  << "7"
                  << "\" (cquery is running with \"" << clang_version << "\")"
                  << std::endl;
        return false;
    }
    ...
    ClangIndex index;
    for (std::string path : GetFilesAndDirectoriesInFolder(
        ...
        if (EndsWithAny(path, {".m", ".mm"})) {
            if (!RunObjectiveCIndexTests()) {
                std::cout << "Skipping \"" << path
                          << "\" since this platform does not "
                          << "support running Objective-C tests." << std::endl;
                continue;
            }

            // objective-c tests are often not updated right away. do not bring
            // down CI if they fail.
            if (!enable_update) is_fail_allowed = true;
        }

        if (path.find(filter_path) == std::string::npos) continue;

        if (!filter_path.empty()) std::cout << "Running " << path << std::endl;
        ...
        if (!AnyStartsWith(flags, "-x")) flags.push_back("-xc++");
        // Use c++14 by default, because MSVC STL is written assuming that.
        if (!AnyStartsWith(flags, "-std")) flags.push_back("-std=c++14");
        optional<AbsolutePath> resource_dir = GetDefaultResourceDirectory();
        if (!resource_dir) ABORT_S() << "Cannot resolve resource directory";
        ...
        for (const auto& entry : all_expected_output) {
            ...
            auto basename = [](const AbsolutePath& path) -> std::string {
                ...
                if (last_index == std::string::npos) return path.path;
            ...
            if (actual == expected) {
                ...
                std::cout << std::endl;
                if (enable_update) {
                    std::cout << "[Enter to continue - type u to update test, "
                                 "a to update all]";
                    char c = 'u';
                    if (!update_all) {
                        c = getchar();
                    ...
                    if (update_all || c == 'u') {
                        ...
                        // |expected_output| because |expected_output| has had
...
TEST_SUITE("ParseTestExpectation") {
    ...
    std::vector<std::string> lines_with_endings = {
        ...
        std::vector<std::string> flags;
        std::unordered_map<std::string, std::string> all_expected_output;
        ParseTestExpectation("foo.cc", lines_with_endings, &text_replacer,
    ...
    TEST_CASE("Apply TEXT_REPLACE") {
        ...
        replacer.replacements.push_back(
            TextReplacer::Replacement{"foo", "bar"});
