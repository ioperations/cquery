    ...
    static std::unique_ptr<TestIndexer> FromEntries(
        ...
        for (const TestEntry& entry : entries) {
            ...
            for (int i = 1; i < entry.num_indexes; ++i) {
                indexes.push_back(std::make_unique<IndexFile>(AbsolutePath(
                    entry.path + "_extra_" + std::to_string(i) + ".h")));
            }

            result->indexes.insert(
                std::make_pair(entry.path, std::move(indexes)));
        }
