...
MAKE_REFLECT_STRUCT(lsTextDocumentClientCapabilities, synchronization,
                    completion, hover, signatureHelp, implementation,
                    references, documentHighlight, documentSymbol, formatting,
                    rangeFormatting, onTypeFormatting, definition, codeAction,
                    codeLens, documentLink, rename);
...
std::string NormalizePathOrAbort(std::string path) {
    ...
    if (!normalized) ABORT_S() << "Unable to find path " << path;
...
struct Handler_Initialize : BaseMessageHandler<In_InitializeRequest> {
    ...
    void Run(In_InitializeRequest* request) override {
        ...
        if (request->params.rootUri) {
...

            {
                ...
                if (!reader.HasParseError()) {
                    ...
                    try {
                        ...
                        // This will not trigger because parse error is handled
                        // in MessageRegistry::Parse in lsp.cc
                    }
                }
                ...
                if (g_config->cacheDirectory.empty()) {
                    ...
                    optional<AbsolutePath> cacheDir = NormalizePath(
                        g_config->cacheDirectory, false /*ensure_exists*/);
                    if (!cacheDir)
            ...
            // Start indexer threads. Start this after loading the project, as
            // that may take a long time. Indexer threads will emit
            // status/progress reports.
            if (g_config->index.threads == 0) {
                // If the user has not specified how many indexers to run, try
                // to guess an appropriate value. Default to 80% utilization.
                const float kDefaultTargetUtilization = 0.8f;
                g_config->index.threads =
                    (int)(std::thread::hardware_concurrency() *
                          kDefaultTargetUtilization);
                if (g_config->index.threads <= 0) g_config->index.threads = 1;
            }
            LOG_S(INFO) << "Starting " << g_config->index.threads
                        << " indexers";
            ...
            time.Reset();
            project->Index(QueueManager::Instance(), working_files,
                           request->id);
