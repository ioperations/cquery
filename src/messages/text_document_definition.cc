        ...
        for (QueryId::SymbolRef sym : FindSymbolsAtLocation(
            ...
            EachEntityDef(db, sym, [&](const auto& def) {
                if (def.spell && def.extent) {
                    ...
                    // If on a definition, clear |uses| to find declarations
                    // below.
                    ...
                    // We use spelling start and extent end because this causes
                    // vscode to highlight the entire definition when previewing
                    // / hoving with the mouse.
        ...
        if (out.result.empty()) {
            ...
            if (!has_symbol) {
                ...
                std::string_view query =
                    LexIdentifierAroundPos(position, buffer);
                ...
                // position, not in the same file, line distance> to find the
                // best match.
                std::tuple<int, int, bool, int> best_score{INT_MAX, 0, true, 0};
                int best_i = -1;
                for (int i = 0; i < (int)db->symbols.size(); ++i) {
                    if (db->symbols[i].kind == SymbolKind::Invalid) continue;
