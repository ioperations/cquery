...
namespace {
MethodType k_method_type = "textDocument/codeAction";
...
optional<int> FindIncludeLine(const std::vector<std::string>& lines,
    ...
    for (int line = 0; line < (int)lines.size(); ++line) {
        ...
        if (current_line_compare == 0) return nullopt;
    ...
    if (best_include_line) return *best_include_line;
    // If |best_include_line| didn't match that means we likely didn't find an
    // include which was lt the new one, so put it at the end of the last
    // include list.
    if (last_include_line) return *last_include_line + 1;
...
optional<QueryId::File> GetImplementationFile(QueryDatabase* db,
        ...
        switch (sym.kind) {
            case SymbolKind::Func: {
                if (const auto* def = db->GetFunc(sym).AnyDef()) {
                    // Note: we ignore the definition if it is in the same file
                    // (ie, possibly a header).
                    if (def->extent) {
                        QueryId::File t = def->extent->file;
                        if (t != file_id) return t;
                    }
                }
                break;
            }
    ...
    for (auto& entry : db->usr_to_file) {
        ...
        if (EndsWith(path.path, ".h") || EndsWith(path.path, ".hpp")) continue;
...
void EnsureImplFile(QueryDatabase* db, QueryId::File file_id,
    ...
    if (!impl_uri.has_value()) {
        ...
        if (!impl_file_id.has_value()) impl_file_id = file_id;
...
optional<LsTextEdit> BuildAutoImplementForFunction(
    QueryDatabase* db, WorkingFiles* working_files, WorkingFile* working_file,
    int default_line, QueryId::File decl_file_id, QueryId::File impl_file_id,
    QueryFunc& func) {
    const QueryFunc::Def* def = func.AnyDef();
    assert(def);
    for (QueryId::LexicalRef decl : func.declarations) {
        if (decl.file != decl_file_id) continue;
        ...
        if (def->declaring_type) {
            ...
            if (const auto* def1 = declaring_type.AnyDef()) {
                ...
                if (ls_type_extent) {
                    ...
                    same_file_insert_end->character +=
                        1;  // move past semicolon.
                }
            }
        }
            ...
            for (QueryId::SymbolRef sym : file.def->outline) {
                switch (sym.kind) {
                    case SymbolKind::Func: {
                        ...
                        if (!def1 || !def1->extent) break;

                        for (QueryId::LexicalRef func_decl :
                             sym_func.declarations) {
                            if (func_decl.file == decl_file_id) {
                                int dist = func_decl.range.start.line -
...
struct InTextDocumentCodeAction : public RequestInMessage {
...
MAKE_REFLECT_STRUCT(InTextDocumentCodeAction::LsCodeActionContext, diagnostics);
MAKE_REFLECT_STRUCT(InTextDocumentCodeAction::LsCodeActionParams, text_document,
                    range, context);
MAKE_REFLECT_STRUCT(InTextDocumentCodeAction, id, params);
REGISTER_IN_MESSAGE(InTextDocumentCodeAction);

struct OutTextDocumentCodeAction
...
struct HandlerTextDocumentCodeAction
    : BaseMessageHandler<InTextDocumentCodeAction> {
    MethodType GetMethodType() const override { return k_method_type; }

    void Run(InTextDocumentCodeAction* request) override {
        ...
        if (!working_file) {
            ...
            LOG_S(WARNING) << "[error] textDocument/codeAction could not find "
                              "working file";
            return;
        }

        OutTextDocumentCodeAction out;
        out.id = request->id;
        ...
        std::vector<QueryId::SymbolRef> syms = FindSymbolsAtLocation(
            working_file, file, request->params.range.start);
        for (QueryId::SymbolRef sym : syms) {
            switch (sym.kind) {
                case SymbolKind::Type: {
                    ...
                    EachDefinedFunc(db, def->funcs, [&](QueryFunc& func_def) {
                        ...
                        if (!command.arguments.edits.empty() &&
                            command.arguments
                                    .edits[command.arguments.edits.size() - 1]
                    ...
                    if (command.arguments.edits.empty()) break;

                    // If we're inserting at the end of the document, put a
                    // newline before the insertion.
                    if (command.arguments.edits[0].range.start.line >=
                        default_line)
                        command.arguments.edits[0].new_text.insert(0, "\n");

                    command.arguments.text_document_uri = *impl_uri;
                    command.title = "Auto-Implement " +
                                    std::to_string(num_edits) + " methods on " +
                                    std::string(def->ShortName());
                ...
                case SymbolKind::Func: {
                    ...
                    OutTextDocumentCodeAction::Command command;
                    command.title =
                        "Auto-Implement " + std::string(def->ShortName());
                    command.command = "cquery._autoImplement";
                    command.arguments.text_document_uri = *impl_uri;
            ...
            if (!out.result.empty()) break;
        }

        std::vector<lsDiagnostic> diagnostics;
        working_files->DoAction(
            [&]() { diagnostics = working_file->m_diagnostics; });
        for (lsDiagnostic& diag : diagnostics) {
            ...
            if (diag.severity == lsDiagnosticSeverity::Error &&
                ...
                for (size_t i = 0; i < db->symbols.size(); ++i) {
                    ...
                    if (!decl_file_id) continue;

                    QueryFile& decl_file = db->files[decl_file_id->id];
                    if (!decl_file.def) continue;
                ...
                for (const std::string& path : include_absolute_paths) {
                    optional<lsCompletionItem> item =
                        include_complete->FindCompletionItemForAbsolutePath(
                            path);
                    if (!item) continue;
                    if (item->textEdit)
                        include_insert_strings.insert(item->textEdit->new_text);
                    else if (!item->insertText.empty())
                        include_insert_strings.insert(item->insertText);
                    else {
                        // FIXME
                        // https://github.com/cquery-project/cquery/issues/463
                        LOG_S(WARNING)
                ...
                if (!include_insert_strings.empty()) {
                    ...
                    for (const std::string& include_insert_string :
                        ...
                        edit.new_text = include_insert_string + "\n";
                        command.arguments.edits.push_back(edit);
                    }
                    ...
                    if (include_insert_strings.size() == 1)
                        command.title =
                            "Insert " + *include_insert_strings.begin();
                    else
                        command.title =
                            "Pick one of " +
                    ...
                    command.arguments.text_document_uri =
...
REGISTER_MESSAGE_HANDLER(HandlerTextDocumentCodeAction);
