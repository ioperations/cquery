        ...
        for (QueryId::SymbolRef sym : file->def->outline) {
            ...
            switch (sym.kind) {
                case SymbolKind::Type: {
                    ...
                    if (!def || def->kind == ls_symbol_kind::Namespace)
                        continue;
                    AddCodeLens("ref", "refs", &common,
                                OffsetStartColumn(ref, 0), type.uses,
                                true /*force_display*/);
                    AddCodeLens("derived", "derived", &common,
                                OffsetStartColumn(ref, 1),
                                GetDeclarations(db, type.derived),
                                false /*force_display*/);
                    AddCodeLens("var", "vars", &common,
                                OffsetStartColumn(ref, 2),
                ...
                case SymbolKind::Func: {
                    ...
                    // For functions, the outline will report a location that is
                    // using the extent since that is better for outline. This
                    // tries to convert the extent location to the spelling
                    // location.
                    auto try_ensure_spelling = [&](QueryId::LexicalRef ref) {
                        optional<QueryId::LexicalRef> def =
                            GetDefinitionSpell(db, ref);
                        if (!def ||
                            def->range.start.line != ref.range.start.line) {
                            return ref;
                        }
                        return *def;
                    };
                    ...
                    if (base_callers.empty() && derived_callers.empty()) {
                        ...
                        if (!base_callers.empty())
                            AddCodeLens("base call", "base calls", &common,
                                        ...
                                        base_callers, false /*force_display*/);
                        if (!derived_callers.empty())
                            AddCodeLens(
                                "derived call", "derived calls", &common,
                                OffsetStartColumn(loc, offset++),
                                derived_callers, false /*force_display*/);
                    }

                    AddCodeLens("derived", "derived", &common,
                                OffsetStartColumn(ref, offset++),
                                GetDeclarations(db, func.derived),
                                false /*force_display*/);

                    // "Base"
                    if (def->bases.size() == 1) {
                        optional<QueryId::LexicalRef> base_loc =
                            GetDefinitionSpell(
                        ...
                        if (base_loc) {
                            ...
                            if (ls_base) {
                                ...
                                if (range) {
                                    ...
                                    code_lens.command =
                                        LsCommand<LsCodeLensCommandArguments>();
                ...
                case SymbolKind::Var: {
                    ...
                    if (!def ||
                        (def->IsLocal() && !g_config->codeLens.localVariables))
                        continue;

                    bool force_display = true;
                    // Do not show 0 refs on macro with no uses, as it is most
                    // likely a header guard.
                    if (def->kind == ls_symbol_kind::Macro)
                        force_display = false;

                    AddCodeLens("ref", "refs", &common,
                                OffsetStartColumn(ref, 0), var.uses,
                                force_display);
                    break;
                }
