...
extern bool g_test_output_mode;
...
struct ProjectConfig {
    ...
    project_mode mode = project_mode::CompileCommandsJson;
    NormalizationCache normalization_cache;
};

const std::vector<std::string>& GetSystemIncludes(
    ProjectConfig* project_config, const std::string& compiler_driver,
    LanguageId language, const std::string& working_directory,
    ...
    // Capture these flags in |extra_flags|, since they may change system
    // include directories.
    static std::vector<std::string> k_flags_to_pass = {
        "--gcc-toolchain", "--sysroot", "-isysroot", "-stdlib", "--target"};
    static std::vector<std::string> k_standalone_flags_to_pass = {
    ...
    for (const std::string& flag : flags) {
        ...
        for (const std::string& to_pass_flag : k_flags_to_pass) {
            if (!StartsWith(flag, to_pass_flag)) continue;
            extra_flags.push_back(flag);
            capture_next = flag.size() == to_pass_flag.size();
            break;
        }

        for (const std::string& to_pass_flag : k_standalone_flags_to_pass) {
            if (flag == to_pass_flag) extra_flags.push_back(flag);
        }
    }
    ...
    if (IsAbsolutePath(compiler_driver)) {
        ...
        compiler_drivers.emplace_back(
            GetExecutablePathNextToCqueryBinary("cquery-clang").path);
    ...
    project_config->discovered_system_defines[language] = FindSystemDefines(
        compiler_drivers, language_string, working_directory, extra_flags);
    ...
    for (const auto& element :
         project_config->discovered_system_defines[language]) {
        project_config->discovered_system_includes[language].emplace_back(
            element);
    }
...
std::vector<std::string> k_path_args = {"-I",
                                        "-iquote",
                                        "-isystem",
                                        "--sysroot=",
                                        "-isysroot",
...
// of kPathArgs.
std::vector<std::string> k_normalize_path_args = {"--sysroot="};

// Arguments whose path arguments should be injected into include dir lookup
// for #include completion.
std::vector<std::string> k_quote_include_args = {"-iquote"};
std::vector<std::string> k_angle_include_args = {"-I", "/I", "-isystem"};

bool ShouldAddToQuoteIncludes(const std::string& arg) {
    return StartsWithAny(arg, k_quote_include_args);
}
bool ShouldAddToAngleIncludes(const std::string& arg) {
    return StartsWithAny(arg, k_angle_include_args);
}

LanguageId SourceFileLanguage(const std::string& path) {
    if (EndsWith(path, ".c")) return LanguageId::C;
    if (EndsWith(path, ".cpp") || EndsWith(path, ".cc")) return LanguageId::Cpp;
    if (EndsWith(path, ".mm")) return LanguageId::ObjCpp;
    if (EndsWith(path, ".m")) return LanguageId::ObjC;
    return LanguageId::Unknown;
}

Project::Entry GetCompilationEntryFromCompileCommandEntry(
    ProjectConfig* config, const CompileCommandsEntry& entry) {
    auto cleanup_maybe_relative_path =
        [&](const std::string& path) -> AbsolutePath {
        // TODO/FIXME: Normalization will fail for paths that do not exist.
        // Should it return an optional<std::string>?
        assert(!path.empty());
        if (entry.directory.empty() || IsAbsolutePath(path)) {
            // We still want to normalize, as the path may contain ..
            // characters.
    ...
    for (const std::string& arg : entry.args) {
        if (arg.compare(0, 3, "%c ") == 0) {
            if (lang == LanguageId::C) args.push_back(arg.substr(3));
        } else if (arg.compare(0, 5, "%cpp ") == 0) {
            if (lang == LanguageId::Cpp) args.push_back(arg.substr(5));
        } else if (arg == "%clang") {
    ...
    // If |compilationDatabaseCommand| is specified, the external command
    // provides us the JSON compilation database which should be strict. We
    // should do very little processing on |args|.
    if (config->mode != project_mode::ExternalCommand && !clang_cl) {
        ...
        while (
            ...
            // There may be other filenames (e.g. more than one source
            // filenames) preceding main source filename. We use a heuristic
            // here. `.` may occur in both command names and source filenames.
            // If `.` occurs in the last 4 bytes of args[i] and not followed by
            // a digit, e.g. .c .cpp, We take it as a source filename. Others
            // (like ./a/b/goma clang-4.0) are seen as commands.
    ...
    if (!g_test_output_mode) {
        std::vector<const char*> platform = GetPlatformClangArguments();
        for (const auto* arg : platform) result.args.push_back(arg);
    }
    ...
    // Note that when processing paths, some arguments support multiple forms,
    // ie,
    ...
    for (; i < args.size(); ++i) {
        ...
        if (!next_flag_is_path) {
            if (StartsWithAny(arg, k_blacklist_multi)) {
                ++i;
                continue;
            }
            if (StartsWithAny(arg, k_blacklist)) continue;
        }
        ...
        if (next_flag_is_path) {
            ...
            if (clang_cl) arg = normalized_arg.path;
            ...
            for (const std::string& flag_type : k_path_args) {
                // {"-I", "foo"} style.
                if (arg == flag_type) {
                ...
                if (StartsWith(arg, flag_type)) {
                    ...
                    AbsolutePath path = cleanup_maybe_relative_path(
                        arg.substr(flag_type.size()));
                    if (clang_cl || StartsWithAny(arg, k_normalize_path_args))
            ...
            // The path needs to be absolute, otherwise clang_codeCompleteAt is
            // extremely slow. See
            ...
            // TODO Exclude .a .o to make link command in compile_commands.json
            // work. Also, clang_parseTranslationUnit2FullArgv does not seem to
            // accept multiple source filenames.
    ...
    for (const auto& flag : config->extra_flags) result.args.push_back(flag);
    ...
    const auto& system_includes = GetSystemIncludes(
        config, compiler_driver, lang, entry.directory, result.args);
    for (const auto& flag : system_includes) result.args.push_back(flag);
...
std::vector<Project::Entry> LoadFromDirectoryListing(
    ProjectConfig* config, bool use_global_config = false) {
    std::vector<Project::Entry> result;
    config->mode = project_mode::DotCquery;
    ...
    if (project_dir_args.empty() && c_cpp_props) {
        config->mode = project_mode::CCppProperties;
        project_dir_args = c_cpp_props.value().args;
        LOG_S(INFO) << "Using c_cpp_properties.json";
    } else if (!project_dir_args.empty()) {
        LOG_S(INFO) << "Using .cquery arguments "
                    << StringJoin(project_dir_args);
    }

    auto get_compiler_argument_for_file =
        [&config, &folder_args, &project_dir_args](const std::string& path) {
            for (std::string cur = GetDirName(path);; cur = GetDirName(cur)) {
                ...
                if (it != folder_args.end()) return it->second;
    ...
    for (const std::string& file : files) {
        ...
        e.file = file;
        e.args = get_compiler_argument_for_file(file);
        if (e.args.empty()) e.args.push_back("%clang");  // Add a Dummy.
...
std::vector<Project::Entry> LoadCompilationEntriesFromDirectory(
    ProjectConfig* project, const std::string& opt_compilation_db_dir) {
    ...
    if (g_config->compilationDatabaseCommand.empty()) {
        project->mode = project_mode::CompileCommandsJson;
        // Try to load compile_commands.json, but fallback to a project listing.
    } else {
        project->mode = project_mode::ExternalCommand;
    ...
    if (!IsAbsolutePath(comp_db_dir)) {
        comp_db_dir = project->normalization_cache.Get(project->project_dir +
                                                       comp_db_dir);
    }
...
int ComputeGuessScore(const std::string& a, const std::string& b) {
    const int k_match_prefix_weight = 100;
    const int k_mismatch_directory_weight = 100;
    const int k_match_postfix_weight = 1;
    ...
    for (i = 0; i < a.size() && i < b.size(); ++i) {
        if (a[i] != b[i]) break;
        score += k_match_prefix_weight;
    }

    // Reduce score based on mismatched directory distance.
    for (size_t j = i; j < a.size(); ++j) {
        if (a[j] == '/') score -= k_mismatch_directory_weight;
    }
    for (size_t j = i; j < b.size(); ++j) {
        if (b[j] == '/') score -= k_mismatch_directory_weight;
    }

    // Increase score based on common ending. Don't increase as much as matching
    // prefix or directory distance.
    for (size_t offset = 1; offset <= a.size() && offset <= b.size();
         ++offset) {
        if (a[a.size() - offset] != b[b.size() - offset]) break;
        score += k_match_postfix_weight;
    }
...
Project::Entry Project::FindCompilationEntryForFile(
    ...
    if (it != absolute_path_to_entry_index_.end()) return entries[it->second];
    ...
    if (!best_entry) {
        ...
        // |best_entry| probably has its own path in the arguments. We need to
        // remap that path to the new filename.
...
void Project::Index(QueueManager* queue, WorkingFiles* working_files,
                    LsRequestId id) {
...
TEST_SUITE("Project") {
    void CheckFlags(const std::string& directory, const std::string& file,
        ...
        for (int i = 0; i < std::min(result.args.size(), expected.size());
             ++i) {
    ...
    TEST_CASE("ycm") {
        CheckFlags(
            "/w/c/s/out/Release",
            "../../ash/login/lock_screen_sanity_unittest.cc",
            ...
            /* expected */
            {"&/w/c/s/out/Release/../../third_party/llvm-build/Release+Asserts/"
             "bin/"
             "clang++",
    ...
    TEST_CASE("Entry inference remaps file names") {
...

        {
            ...
            REQUIRE(entry.has_value());
            REQUIRE(entry->args ==
                    std::vector<std::string>{"a", "b", "ee.cc", "d"});
        }
    }
    ...
    TEST_CASE("Entry inference prefers same file endings") {
        ...
        {
            optional<Project::Entry> entry = p.FindCompilationEntryForFile(
                AbsolutePath("my_browsertest.cc"));
            REQUIRE(entry.has_value());
            REQUIRE(entry->args == std::vector<std::string>{"arg1"});
        }
        {
        ...
        {
            optional<Project::Entry> entry = p.FindCompilationEntryForFile(
                AbsolutePath("common/my_unittest.cc"));
            REQUIRE(entry.has_value());
            REQUIRE(entry->args == std::vector<std::string>{"arg2"});
        }
