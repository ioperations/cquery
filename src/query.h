...
struct QuerySymbolRef : Reference {
    ...
    QuerySymbolRef(Range range, AnyId id, SymbolKind kind, enum role role)
...
struct QueryLexicalRef : Reference {
    ...
    QueryLexicalRef(Range range, AnyId id, SymbolKind kind, enum role role,
                    Id<QueryFile> file)
...
MAKE_REFLECT_STRUCT(QueryFile::Def, file, path, args_hash, language, outline,
                    all_symbols, inactive_regions, dependencies);

template <typename TDerived, typename TDefinitionData>
struct QueryEntity {
    using Def = TDefinitionData;
    using DefUpdate = WithId<Id<TDerived>, Def>;
    using DeclarationsUpdate =
        MergeableUpdate<Id<TDerived>, QueryId::LexicalRef>;
...
struct IdMap {
  ...
  std::vector<typename IndexToQuery<I>::type> ToQuery(const std::vector<I>& a) const {
   ...
   private:
    spp::sparse_hash_map<IndexId::Type, QueryId::Type> m_cached_type_ids;
    spp::sparse_hash_map<IndexId::Func, QueryId::Func> m_cached_func_ids;
    spp::sparse_hash_map<IndexId::Var, QueryId::Var> m_cached_var_ids;
};
